...
> **Учётный контур.** Платформа обязана вести полный документооборот маркетинга
> (акции, публикации, договоры, акты, счета, платежи, бюджеты на рекламу) на базе
> учётной системы **UNA.md** — как через интеграцию, так и с возможностью ручного ввода.
> Требования вынесены в отдельный документ: [**ТЗ, часть II — Документооборот на базе UNA.md**](./TZ-Documentooborot-UNA.md).
...
- Замена бухгалтерского учёта: платформа не ведёт собственный финансовый учёт, а работает поверх UNA.md (часть II);
...
### 7.7 Фаза 6 — Учёт и документооборот
Плейбуки этой фазы работают с учётным контуром UNA.md. Полный список и обязательные
финансовые guardrails — в [части II, §29](./TZ-Documentooborot-UNA.md).

| Код | Название | Периодичность |
|---|---|---|
| `60-campaign-launch` | Запуск акции: проверка бюджета, креативы, медиаплан, UTM | По акции |
| `61-campaign-close` | Закрытие акции: факт, ROI, отчёт, акты | По акции |
| `62-budget-draft` | Проект маркетингового бюджета на период | Ежеквартально |
| `63-adspend-import` | Импорт расходов из рекламных кабинетов | Ежедневно |
| `64-invoice-intake` | Распознавание и предзаполнение входящих счетов/актов | По событию |
| `65-contractor-review` | Оценка эффективности подрядчиков и площадок | Ежемесячно |
| `66-doc-hygiene` | Поиск незакрытых и просроченных документов | Еженедельно |
| `67-plan-fact-alert` | Контроль освоения бюджета, алерты 80/100% | Ежедневно |
