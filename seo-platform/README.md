...
Инструментов пока два: `mcp-site` (чтение страниц, строго по
`network_allowlist` плейбука) и `mcp-lang` (диакритика, длина мета-тегов,
плотность ключа). Если плейбук требует инструмент, которого нет, запуск
**отклоняется**: без доступа к выдаче сессия выдаст правдоподобный вымысел
вместо данных. Частичный прогон возможен явным согласием
(`RUNNER_ALLOW_MISSING_TOOLS=true`) — тогда агент обязан помечать недоступное
как `[TODO]` и перечислять в `open_questions`.

Не реализовано: планировщик по расписанию, остальные коннекторы (`mcp-serp`,
`mcp-gsc`, `mcp-social`, `mcp-una`), генерация проводок через `UN$GFC`
...
npm test               # 109 тестов: движок + API + раннер на настоящем Postgres в WASM
