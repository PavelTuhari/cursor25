...
| Коннекторы | `apps/api/src/runner/connectors` | Выдача (SERP) и Google Search Console |
...
Не реализовано: коннекторы соцсетей и рекламных кабинетов (`mcp-social`,
`mcp-ads`), шлюз `mcp-una` для раннера, генерация проводок через `UN$GFC`
...
npm test               # 150 тестов: движок + API + раннер на настоящем Postgres в WASM
