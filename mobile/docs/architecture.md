...
```mermaid
flowchart LR
  P["Карточка товара"] --> C["cart_items (только устройство)"]
  C --> K["Оформление: способ, интервал, оплата"]
  K --> O["orders.saveLocal → _outbox"]
  O --> S["Синхронизация: PUT /account/orders/{id}"]
  S --> B["Бэкенд присваивает номер и статус"]
  B --> O2["Ответ пишется поверх локальной строки"]
  O2 --> U["Экран заказа: прогресс, отмена, повтор"]
```

Корзина — это таблица с `direction: "local"`: она не уходит на сервер и не участвует в
синхронизации вообще. Заказ, наоборот, обычная двусторонняя сущность, поэтому
оформление работает офлайн: строка пишется локально, задача встаёт в очередь, а номер
и подтверждение приезжают, когда появляется связь.

Купон применяется до расчёта доставки и минимальной суммы, как в кассовом чеке, и
может «отодвинуть» бесплатную доставку — это покрыто тестом.

## Push-уведомления

Регистрация токена — это правила, а не вызовы SDK: `PushService` работает через
`PushAdapter`, поэтому «когда спрашивать разрешение», «когда повторно отправлять
токен» и «что делать при выходе» проверяются тестами без устройства. Подпись
`токен|пользователь|язык` хранится в `_kv`, и пока она не изменилась, повторных
запросов к бэкенду нет.

Маршрут из уведомления проходит через белый список экранов: полезная нагрузка приходит
с сервера, поэтому произвольный `screen` или объект вместо параметра просто
отбрасываются.
