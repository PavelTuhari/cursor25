...
| `features` | фич-флаги: `loyalty`, `shoppingList`, `favorites`, `stores`, `promoFlyers`, `search`, `account`, `purchaseHistory`, `eReceipts`, `cart`, `onlineOrders`, `coupons`, `push`, `scanner` |
...
| `cart` | способы получения, минимальная сумма, стоимость и порог бесплатной доставки, интервалы, лимит количества |
| `push` | включение, эндпоинт регистрации токена, спрашивать ли разрешение на первом запуске, темы |
| `scanner` | форматы штрихкодов, открывать карточку или сразу добавлять в список |
...
| `cart` | корзина с итогами и кнопкой оформления | `showCoupon`, `checkoutAction` |
| `order_list` | список заказов | `source` |
| `coupon_strip` | персональные купоны | `source`, `layout` (`carousel`/`list`), `moreAction` |
...
| Поле | Значения |
|------|----------|
| `direction` | `pull` (только с сервера), `push`, `bidirectional`, `local` (только на устройстве, не синхронизируется — так устроена корзина) |
...
"cart": {
  ...
  "slotDays": 3,
  "slotHours": ["10:00-12:00", "12:00-14:00"],
  "maxQuantityPerItem": 30
}
```

Оставьте `"fulfillment": ["pickup"]` — и экран оформления не покажет ни адрес, ни
интервалы. `freeDeliveryFrom: 0` отключает бесплатную доставку; валидатор
предупредит, если порог бесплатной доставки ниже минимальной суммы заказа (тогда
доставка бесплатна всегда).

Скидка купона вычитается **до** проверки минимальной суммы и порога бесплатной
доставки — как в кассовом чеке.
...
в конфигурации — код менять не нужно. Купоны и заказы отмечены так же, как профиль и
чеки.

Корзина (`cart_items`) намеренно не помечена: это данные устройства с
`direction: "local"`, они не уходят на сервер и переживают выход из аккаунта.

## Веб-цель

`metro.config.js` добавляет `.wasm` в ассеты и выставляет заголовки cross-origin
isolation: без этого `expo-sqlite` не соберётся под веб (SQLite там работает через
WebAssembly и SharedArrayBuffer). Веб-сборка нужна не для продакшена, а для быстрого
просмотра экранов и снятия скриншотов — см. раздел «Ручной прогон» в README.

## Новый экран — чек-лист

1. Положите JSON в `config/screens/<id>.json`.
2. Добавьте импорт в `src/config/defaults.ts` (бандлер требует явного импорта; тест
   `bundles every screen that exists on disk` поймает забытый).
3. Если на экран нужно переходить — добавьте маршрут в `src/ui/navigation/RootNavigator.tsx`
   и, при необходимости, вкладку в `navigation.config.json`.
4. `npm run validate-config` проверит блоки, запросы и ключи переводов.
